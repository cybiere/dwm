...
drawbar(Monitor *m)
{
	...
	if (m == selmon) { /* status is only drawn on selected monitor */
		...
		drw_text(drw, m->ww - tw - 2 * sp - stw , 0, tw, bh, lrpad / 2 - 2, stext, 0);
	}
	...
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeTagSel : occ & 1 << i ? SchemeNorm : SchemeTagNorm]);
		drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
		if (occ & 1 << i)
			drw_rect(drw, x + boxw, bh - boxw, w - ( 2 * boxw + 1), boxw,
	...
	if ((w = m->ww - tw - stw - x) > bh) {
		if (n > 0) {
			...
			for (c = m->clients; c; c = c->next) {
				...
				drw_setscheme(drw, scheme[m->sel == c ? SchemeSel : SchemeTagNorm]);
...
tile(Monitor *m)
{
	...
	if(n == 1){
		...
		resize(c, m->wx + m->gappx, m->wy + m->gappx, m->ww - 2 * c->bw - 2*gappx, m->wh - 2 * c->bw -2*gappx, 0);
		return;
	}
...
updatesystray(void)
{
	...
	x -= w;
	XMoveResizeWindow(dpy, systray->win, x , y + vp, w, bh);
